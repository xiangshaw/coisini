...
import cn.coisini.system.mapper.AsyncLoginLogMapper;
...
public class AsyncLoginLogServiceImpl extends ServiceImpl<AsyncLoginLogMapper, AsyncLoginLog> implements AsyncLoginLogService {
     ...
     * @param message       消息内容
     */
    public void recordLoginLog(Long userId, String username, Boolean status, String loginIp, String loginIpSource, String loginIpCity,String message) {
    ...
    public Result<AsyncLoginLog> pagingQuery(AsyncLoginLogQueryVo asyncLoginLogQueryVo) {
        ...
        return Result.ok(new PageUtils(loginLogPage));
    }

    // 根据id查询登录日志
    @Override
    public Result<AsyncLoginLog> get(Long id) {
        // 1.检查参数
        if (id == null) {
    ...
    @Override
    public Result<AsyncLoginLog> removeLoginLogById(Long id) {
        // 1.检查参数
        if (id == null) {
            return Result.error(ResultEnum.PARAM_INVALID);
        }
    ...
    // 批量删除登录日志
    @Override
    public Result<AsyncLoginLog> batchRemove(List<Long> ids) {
        // 1.检查参数
        if (ids == null) {
            return Result.error(ResultEnum.PARAM_INVALID);
        }
