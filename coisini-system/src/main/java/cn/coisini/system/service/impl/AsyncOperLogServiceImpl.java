...
import cn.coisini.system.mapper.AsyncOperLogMapper;
...
public class AsyncOperLogServiceImpl extends ServiceImpl<AsyncOperLogMapper, AsyncOperLog> implements AsyncOperLogService {
    ...
    public Result<AsyncOperLog> pagingQuery(AsyncOperLogQueryVo asyncOperLogQueryVo) {
        ...
        return Result.ok(new PageUtils(operLogPage));
    }

    // 根据id查询操作日志
    @Override
    public Result<AsyncOperLog> get(Long id) {
        // 1.检验id
        if (id == null) {
    ...
    // 根据id删除操作日志
    @Override
    public Result<AsyncOperLog> removeOperLogById(Long id) {
        // 1.检查参数
        if (id == null) {
            return Result.error(ResultEnum.PARAM_INVALID);
        }
    ...
    // 批量删除操作日志
    @Override
    public Result<AsyncOperLog> batchRemove(List<Long> ids) {
        // 1.检查参数
        if (ids == null) {
            return Result.error(ResultEnum.PARAM_INVALID);
        }
